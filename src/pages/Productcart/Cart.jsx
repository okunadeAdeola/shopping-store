...
const Cart = () => {
  ...
  return (
    <div className="container mx-auto p-4 bg-amber-100 sm:p-6">
      {store.length > 0 ?
        <>
          <div className="flex flex-col lg:flex-row justify-between gap-8">
            <div id="folder" className="lg:w-3/5 w-full bg-gray-100 overflow-y-auto max-h-[500px] p-4 sm:p-6 rounded-lg shadow-md">
              ...
              <div className="flex justify-end mb-4 space-x-2">
                ...
                <FaList onClick={() => setViewMode('grid')} className='cursor-pointer text-orange-500' />
              </div>
              ...
              {cartData.length === 0 ? (
                ...
                <div className={viewMode === 'list' ? 'space-y-4' : 'grid grid-cols-1 sm:grid-cols-2 gap-4'}>
                  {cartData.map((item, i) => (
                    <div
                      key={i}
                      className={`flex ${viewMode === 'list' ? 'justify-between bg-amber-50 shadow-lg rounded p-4 sm:p-5 items-center' : 'flex-col bg-amber-50 rounded shadow-lg p-5 '} border-b pb-4`}
                    >
                      <div className={`${viewMode === 'list' ? 'flex items-center gap-4' : 'text-center'}`}>
                        ...
                        <div>
                          ...
                          <p className="text-sm text-orange-600">Quantity: {item?.cartQuantity}</p>
                        </div>
                      </div>
                      ...
                      <div className={`${viewMode === 'list' ? 'text-right' : 'flex flex-col items-center'}`}>
                        ...
                        <div className="flex items-center space-x-2 mt-2">
                          <button
                            ...
                            className={`px-2 py-1 rounded ${item.cartQuantity === 1 ? 'bg-orange-100' : 'bg-gray-200 hover:bg-gray-300'}`}
                          >
                            -
                          </button>
                          <p className="text-orange-600">{item?.cartQuantity}</p>
                          <button
            ...
            {/* Checkout Section */}
            <div className="lg:w-2/5 w-full bg-amber-50 p-4 sm:p-6 rounded-lg h-full shadow-lg">
              <h2 className="text-xl sm:text-2xl font-semibold mb-4">Checkout</h2>

              {/* Total Price */}
              <div className="flex justify-between mb-4">
                <p className="text-lg">Subtotal</p>
                <p className="text-lg font-semibold text-orange-600">${cartQuantity}</p>
              </div>
              ...
              <button onClick={() => handleCheckout()} className="w-full bg-orange-600 text-amber-50 py-3 rounded-lg hover:bg-black transition-colors font-semibold">
        ...
        :
        <div className="text-center shadow-lg border border-amber-50 bg-amber-50">
          ...
          <button className='bg-orange-500 p-2 mb-5 rounded text-amber-50'>START SHOPPING</button>
        </div>
      }
      <Tops />
      <Footer />
